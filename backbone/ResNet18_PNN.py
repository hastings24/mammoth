...
class ResNet_PNN(ResNet):
    ...
    def __init__(self, block: BasicBlock, num_blocks: List[int],
        ...
        """
        super(ResNet_PNN, self).__init__(block, num_blocks, num_classes, nf)
        if old_cols is None:
            old_cols = []
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if len(self.old_cols) == 0:
            return super(ResNet_PNN, self).forward(x)
        else:
            with torch.no_grad():
...
def ResNet18_PNN(nclasses: int, nf: int=64,
    ...
    if old_cols is None:
        old_cols = []
    return ResNet_PNN(BasicBlockPnn, [2, 2, 2, 2], nclasses, nf,
